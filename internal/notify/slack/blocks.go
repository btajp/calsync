...
const (
	// maxDigestEventSections: 表示する予定(section)はSlackの実用上限に合わせ最大 20 件。
	// 超過分はトップレベル blocks の context「…他 N 件」に集約する
	// (46 件キャップは v2.1 で 20 に変更。v2.2 で attachment は連続同色をまとめるため
	// 上限は「予定数」で管理する。v2.2 スペック 5 章)
	maxDigestEventSections = 20
...
// digestMessage はダイジェストの (blocks, attachments) を組み立てる(v2.2 スペック 5 章)。
// トップレベル blocks は header + 必要時の context(他 N 件・取得失敗)のみ。
// 時系列順の連続する同一色アカウント(colorAccountOf)を 1 attachment に束ねる
// (run-length グルーピング。並べ替えはしない)。colorFor はアカウント ID → 16 進色。
func digestMessage(day time.Time, entries []engine.DigestEntry, failedAccounts []string, loc *time.Location, colorFor func(string) string) ([]block, []attachment) {
	...
	shown := entries
	if len(shown) > maxDigestEventSections {
		shown = shown[:maxDigestEventSections]
	}
	attachments := make([]attachment, 0, len(shown))
	var groupAccount string
	for i, e := range shown {
		sec := eventSection(e, d, loc)
		acct := colorAccountOf(e)
		if i > 0 && acct == groupAccount {
			last := &attachments[len(attachments)-1]
			last.Blocks = append(last.Blocks, sec)
			continue
		}
		groupAccount = acct
		attachments = append(attachments, attachment{Color: colorFor(acct), Blocks: []block{sec}})
	}
