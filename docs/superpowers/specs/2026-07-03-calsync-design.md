...
2. **set-difference**: フル同期結果とイベントキャッシュ/mappings を突合(alive 判定にもループ遮断を適用 — 自作ブロッカーは origin として「生存」に数えない)。加えて「フル同期が成功したカレンダーの origin なのにキャッシュに存在しない active mapping」は汚染とみなし blocker ごと掃除する。消えた元予定 → ブロッカー削除、新規範囲入り → ブロッカー作成、範囲外に出たブロッカー → 掃除。**Google の「410 時は全ワイプ」公式手順は mappings に適用しない**(適用するとループ防止と対応関係が失われる)。破棄するのはカーソルとイベントキャッシュのみ
