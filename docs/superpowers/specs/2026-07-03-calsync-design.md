...
| OAuth スコープ | Google: `calendar.events` / Microsoft: `Calendars.ReadWrite` + `offline_access` | 最小権限。freebusy 系スコープでは差分同期に必要な events 読み取りができない |
...
- 終日イベントは終日ブロッカーとしてミラーする。Google は `start.date`/`end.date`(end は排他的 = 翌日)、Graph は `isAllDay=true` + **ターゲットカレンダーのタイムゾーンでの midnight 境界**(公式要件。UTC midnight だと日付がズレる)。ターゲットの TZ はカレンダー設定から取得してキャッシュする
