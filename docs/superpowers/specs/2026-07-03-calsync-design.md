...
日次(既定 04:00、コンテナのローカル TZ)+ `calsync reconcile` で手動実行。1 回のリコンサイルは次のフェーズ順で行う(**フェーズ0が最初であることが安全性の要**):

0. **タグからの mappings 先行再構築**: 全カレンダーの calsync タグ付きブロッカーを列挙し、mappings に無いものを先に再収容する(削除判断はしない)。Graph delta はタグを返せないため、DB 全損時にこれを配布より先に行わないと Microsoft カレンダー上の受領ブロッカーが実予定と誤認され全カレンダーへ再ミラーされる(実障害 2026-07-04: 複製957件 → 本フェーズ導入後のリコンサイル1回で全自動除去を確認)

1. **カーソル張り直し**: 全カレンダーで新ウィンドウ(now 〜 now+window)のフル同期を実行し、新カーソルを確立(= ウィンドウのスライド)
2. **set-difference**: フル同期結果とイベントキャッシュ/mappings を突合(alive 判定にもループ遮断を適用 — 自作ブロッカーは origin として「生存」に数えない)。加えて「フル同期が成功したカレンダーの origin なのにキャッシュに存在しない active mapping」は汚染とみなし blocker ごと掃除する。消えた元予定 → ブロッカー削除、新規範囲入り → ブロッカー作成、範囲外に出たブロッカー → 掃除(**未来側のみ。2026-08-05 変更**: ウィンドウ開始より前に終わった予定のブロッカー・mappings・キャッシュ行は保持する — 「過去方向は同期しない」は“過去向けに作らない”ことであり“終わった予定のブロッカーを消す”ことではない。判定は `EndsBeforeWindow`(キャッシュ上の最終版の時刻)。ウィンドウ内から過去へ移動された場合は従来どおり削除。手で消された過去のブロッカーは復元しない。既知の限界①: ウィンドウ外の削除通知が届かない経路〔Graph delta・カーソル失効後の窓付き再同期〕では、実カレンダーから削除された過去予定のブロッカーが残る〔手動削除で対応〕。既知の限界②: DB 全損からの再構築では過去分の mappings を復元する手段がなく〔ListBlockers は両プロバイダとも時刻下限付きで過去を列挙しない〕、過去ブロッカーは「管理外の履歴」としてカレンダーに残る — 削除追従・accounts remove の対象外になる)。ListBlockers は Google の TimeMin に加え、Graph も `end/dateTime ge <ウィンドウ開始>` の $filter 下限を持つ(2026-08-05。過去ブロッカーの永久蓄積でリコンサイルの Graph GET 数が無限成長するのを防ぐ。組合せ $filter が 400 の場合は時刻条件なしへフォールバックするが、クライアント側の時刻下限は常時適用されるため「過去を列挙しない」契約は保たれ、増えるのは列挙 API コストのみ)。ローカル状態(events キャッシュ・mappings・suppressed 行)は過去分が保持され運用期間に対して線形成長する(SQLite の行数増のみで API コストは増えない。許容判断)。suppressed 昇格(promoteSuppressed / reevaluateSuppressed)と pending 解決にも過去側ガードがあり、過去向けに新規ブロッカーは作らない。**Google の「410 時は全ワイプ」公式手順は mappings に適用しない**(適用するとループ防止と対応関係が失われる)。破棄するのはカーソルとイベントキャッシュのみ
...
| 同一カレンダーの持続的な同期失敗(5回連続) | カーソル毒化とみなし FullResync でカーソルを再初期化(Graph が 410/syncStateNotFound ではなく持続的な 504 を返し続けるケースを実運用で確認・2026-07-04) |
...
- [ ] Graph `/me/events` の `$filter` で拡張プロパティ(singleValueExtendedProperties/Any)と `end/dateTime ge` の組合せが受理されるか(2026-08-05 追加。拒否〔400〕時は時刻条件なしへ自動フォールバックする実装のため動作は阻害しないが、受理されない場合は過去ブロッカー蓄積による ListBlockers コスト増が解消されない)
...
7. ~~Google: 削除済みブロッカーの 409 蘇生~~ → **実測済み(2026-07-04)**: 手動削除したブロッカー(繰り返し予定インスタンス由来)がリコンサイルの restoreMissingBlockers → 409 → events.update で status=confirmed に蘇生し再表示されることを確認
8. Graph: ブロッカー削除後に同一 `transactionId` で再作成した場合の挙動(409 が返るなら復元はエラーとして表面化し日次リコンサイルで再試行される — 許容だが実測で確認)
9. ~~Graph: 終日イベントの UTC 変換~~ → **実測済み(2026-07-04)**: busy 終日イベント(JST 7/4)が `all_day_start=2026-07-04` のまま正しく取り込まれ、日付ズレなし
...
11. ~~`docker build` と `docker run` の成功~~ → **実測済み(2026-07-04)**: イメージ 33.5MB、`docker compose up -d` での常駐稼働・毎分ティック・エラー分離まで確認
