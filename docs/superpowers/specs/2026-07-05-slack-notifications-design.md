...
**実測記録(2026-07-05)**: チャンネル宛てダイジェストの E2E を実環境で確認 — 発火時刻どおり(12:45 設定 → 12:45 投稿)、ライブ取得 6 アカウント全成功(Google×4 / Graph×1 / 併走の通常同期では Graph の一時 504 が発生していたがダイジェスト経路は成功)、0 件日の文言・曜日表示・重複投稿なし(= `chat.postMessage` の channel 文字列応答が成功として処理されている)を確認。スパイク 1 は「取得成功」まで確認済みで、**件名の実表示は初回の予定あり日のダイジェスト(または リマインド)で確認して消し込む**。スパイク 3(DM)は現構成がチャンネル宛てのため未実測。スパイク 4 は実エラー未遭遇のため未実測。
