...
detail_sync:
  - from: work-ms      # origin アカウント id(YAML の accounts[].id)
...
| 同一 (from, to) の重複エントリ禁止 | `config: detail_sync[1]: duplicate pair "work-ms" => "personal"` |
...
(記録中のアカウント id は公開用の仮名。以降の work-a / work-b も同一環境の同じアカウントを指す)
...
- 既存ペア(visibility 未指定 = private)はアップグレード無風(保存ハッシュ不変 — 稼働中の既存ペアに一斉 patch は走らない)
