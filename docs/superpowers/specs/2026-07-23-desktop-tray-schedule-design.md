...
- 更新周期: 1 分ごとに表示を再計算。イベントデータは App.tsx(Shell)が 5 分ごとに独立取得して供給する(`/api/events` を今日〜+7 日で取得。CalendarView の表示ウィンドウ取得とは別経路であり、カレンダータブを開いた/閉じた操作には連動しない)。この整形(`nextEventLabel(events, now)`)は純関数として export し vitest でテストする
...
- 今日のグループ内に「現在時刻」の赤い水平線を表示する(2026-07-28 実機フィードバック反映。位置は純関数 `nowLineIndex`: 終日と開始済みの項目の直後 = これから始まる最初の項目の直前。再レンダリングのたびに現在時刻で再計算)
- 予定取得は stale-while-revalidate に対応: `stale: true` の応答を受けたら約 4 秒後に 1 回だけ再取得する(カレンダービュー設計 2026-07-21 §4 参照)
