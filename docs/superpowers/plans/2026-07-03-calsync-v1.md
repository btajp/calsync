...
- module は `github.com/btajp/calsync`、Go 1.25、`CGO_ENABLED=0`(distroless 配布のため)
...
リポジトリルート(`/Users/user/ghq/github.com/btajp/calsync`)で実行する。コントラクトの固定値どおり module パスは `github.com/btajp/calsync`、Go バージョンは 1.25 に固定する(ローカルの toolchain が新しくても `go mod edit -go=1.25` で明示的に揃える)。

Run:
```bash
go mod init github.com/btajp/calsync
go mod edit -go=1.25
```
Expected:
```
go: creating new go.mod: module github.com/btajp/calsync
```
(`go.mod` に `module github.com/btajp/calsync` と `go 1.25` が入る)
...
```
# github.com/btajp/calsync/internal/model [github.com/btajp/calsync/internal/model.test]
...
FAIL	github.com/btajp/calsync/internal/model [build failed]
```
...
PASS
ok  	github.com/btajp/calsync/internal/model	0.xxxs
```
...
```
# github.com/btajp/calsync/internal/config [github.com/btajp/calsync/internal/config.test]
internal/config/config_test.go:29:38: undefined: Config
internal/config/config_test.go:229:15: undefined: Load
FAIL	github.com/btajp/calsync/internal/config [build failed]
```
...
PASS
ok  	github.com/btajp/calsync/internal/config	0.xxxs
```
...
前提: Task 1 で `go.mod`(module `github.com/btajp/calsync`、Go 1.25)と `internal/model/model.go` が作成済みであること。
...
Expected: FAIL(ビルドエラー。実装ファイル `store.go` が存在しないため、`internal/store/store_test.go` のコンパイルが `undefined: Store` / `undefined: Open` / `undefined: ErrLocked` / `undefined: CalendarState` で失敗し、`FAIL github.com/btajp/calsync/internal/store [build failed]` が出る)
...
Expected: PASS(`TestOpen_CreatesSchemaAndWAL` / `TestOpen_SecondOpenReturnsErrLocked` / `TestCalendars_UpsertAndGet` / `TestCalendars_Mutations` の全サブテスト / `TestCalendars_ListAndDeleteForAccount` がすべて `--- PASS`、`ok github.com/btajp/calsync/internal/store`)
...
Expected: PASS(`ok github.com/btajp/calsync/internal/store`)
...
Expected: FAIL — ビルドエラー。`internal/provider/fake` に非テストの Go ファイルがないため `no non-test Go files in ./internal/provider/fake`(および `github.com/btajp/calsync/internal/provider` パッケージ未解決)が出る
...
**Interfaces:**
- Consumes:
  - Task 1 の `go.mod`(module `github.com/btajp/calsync`、依存 `golang.org/x/oauth2` / `github.com/stretchr/testify`)
...
```
# github.com/btajp/calsync/internal/auth [github.com/btajp/calsync/internal/auth.test]
...
FAIL	github.com/btajp/calsync/internal/auth [build failed]
```
...
PASS
ok  	github.com/btajp/calsync/internal/auth	0.3s
```
...
PASS
ok  	github.com/btajp/calsync/internal/auth	3.1s
```
...
```
# github.com/btajp/calsync/internal/provider/google [github.com/btajp/calsync/internal/provider/google.test]
./changes_test.go:69:35: undefined: Client
./changes_test.go:71:7: undefined: New
FAIL	github.com/btajp/calsync/internal/provider/google [build failed]
```
...
PASS
ok  	github.com/btajp/calsync/internal/provider/google
```
