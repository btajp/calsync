...
- 過去方向の同期はしません。同期対象は現在〜未来 3 ヶ月(`sync_window` で変更可)です。作成済みのブロッカーは予定が終わった後も削除されず残ります(履歴として保持。実カレンダー側で元予定が削除された場合は、削除通知が届く経路では追従して削除されます — Microsoft の差分同期はウィンドウ外の削除を通知しないため、過去の予定の削除には追従できず手動削除が必要です)
...
- **できること**: ダッシュボード(デーモン状態・アカウント別の最終同期・doctor 実行・launchd の停止/起動/再起動・**フルリコンサイルの実行**)、カレンダー(全アカウントの実予定を週/月/スケジュール(リスト)ビューで 1 つに重ねて表示。ブロッカーは除外・free の予定や digest 専用カレンダーの予定は含む。アカウントごとに色分けし凡例を表示、週/日ビューには現在時刻の水平線・今日の強調表示・「今日」ジャンプボタンあり。同じ時刻に始まる複数の予定は 1 つの箱にまとめて全幅の行リストで表示(件数バッジ・行クリックで各予定の詳細)、予定にマウスを乗せると全幅に展開して全文を確認できる。イベントクリックでアプリ内モーダルに詳細(タイトル・日時・アカウント・説明文。説明文中の https リンクは既定ブラウザで開ける)を表示し、会議 URL があればタイトル横の「会議に参加」ボタンで参加(Zoom の会議はブラウザを経由せず Zoom デスクトップアプリを直接起動。未インストール時はブラウザにフォールバック)、「カレンダーで開く」で元カレンダーをブラウザ表示。**launchd 管理下限定**)、設定(`data/calsync.yaml`)のフォーム編集(コメントを保持したまま保存・外部変更を mtime で検出・保存後は再起動を誘導)、アカウント追加ウィザード(前提チェック → OAuth 認可 → カレンダー選択 → YAML 追記 → 再起動誘導。完了画面からフルリコンサイルの即時実行も可能)、自動更新(起動時に GitHub Releases の最新版を確認し、ヘッダのバナーから署名済みアップデートをダウンロード → 再起動。ヘッダの「更新確認」リンクから手動確認も可能)、**メニューバー常駐**(トレイアイコンに次の予定(60分未満は「N分後」、当日は時刻、翌日は「明日HH:MM」、7日以内は「M/D」)を表示。クリックで今日〜+7日のスケジュール一覧をポップオーバー表示 — 開くと今日の開催中/次の予定まで自動スクロールし、今日のグループには現在時刻の赤い水平線が入ります。会議 URL がある予定は行の「参加」ボタンで直接参加でき、項目クリックでパネル内に詳細表示(「← 戻る」でリストへ)、「アプリを開く」でメインウィンドウを表示、「終了」でアプリ全体を終了)
- **できないこと**: アカウントの削除(危険度が高いため非対応。[アカウントの削除](#アカウントの削除)または `calsync-uninstall` スキルを使ってください)、`calsync sync` の手動実行(単発の差分同期。フルリコンサイルはダッシュボード/アカウント追加ウィザードから実行できます)
- **ウィンドウを閉じてもアプリは終了しません**: メインウィンドウの ×(閉じる)はメニューバーへの格納(hide)で、常駐が続きサイドカーも動き続けます。アプリ全体を終了するにはメニューバーのアイコンをクリックして開くポップオーバーの「終了」ボタンを使ってください
- **メニューバー常駐は macOS 14 以降を推奨**: 非表示中のウィンドウでもタイマー処理を止めない `backgroundThrottling` の無効化は macOS 14 以降でのみ有効です(それ未満のバージョンでは no-op)。macOS 14 未満では、メインウィンドウを閉じてメニューバーに格納している間は OS がタイマーを止めてしまい、トレイのタイトルやポップオーバーの予定一覧が次にウィンドウを表示するまで更新されないことがあります
- **カレンダータブは launchd 管理下限定**: ブロッカー除外の判定に mappings(SQLite・読み取り専用)へのアクセスが要るため。manual モード(launchd 未導入)では画面自体は開けますが、予定の取得時に launchd 管理外である旨のエラーメッセージが表示されます(コンテナ運用は下記のとおりアプリ全体が案内表示モードになるため、そもそもこのタブへ到達しません)。launchd 導入手順は本 README の「macOS ネイティブ常駐(推奨)」節を参照してください
- **メニューバー常駐のトレイタイトル・ポップオーバーの予定一覧も launchd 管理下限定**: カレンダータブと同じ `/api/events` を使うため。manual/コンテナ運用ではトレイはアイコンのみ(タイトルなし)になり、ポップオーバーを開くとエラーメッセージが表示されます
- **フルリコンサイルの実行は launchd 管理下限定**: ダッシュボードの「リコンサイル実行」ボタン、またはアカウント追加ウィザード完了画面のボタンから実行できます(確認ダイアログあり)。内部的には稼働中のデーモンを一時停止(`launchctl bootout`)→ `calsync reconcile` をサブプロセス実行(stdout/stderr を `data/reconcile-<UTC時刻>.log` に保存)→ 成否に関わらず再開(`launchctl bootstrap`)という順で行われ、数分かかります。実行中は全タブに専用バナーが表示され、設定の保存・デーモン制御・再実行はできません。失敗した場合はバナーにログファイルのパスが表示されます
...
[GitHub Releases](https://github.com/btajp/calsync/releases) の `desktop-v*` リリースから `calsync_<version>_aarch64.dmg` をダウンロードし、マウントして `calsync.app` を `/Applications` へコピーしてください(Developer ID 署名+公証済みのため Gatekeeper の警告なしに起動できます。Apple Silicon 専用)。以後の更新はアプリ内の自動更新で配信されます。
...
- 同一アカウント内で `calendars` との重複、`blocker_calendar` との重複、`digest_calendars` 内の重複、空文字列はいずれも設定エラーになります
...
全アカウントに対して実行すると、配布済み・受領済みのブロッカーがすべてのカレンダーから削除されます。認証切れ等で消せないアカウントは `--force` でスキップできますが、そのカレンダーに残ったブロッカーは手動削除が必要です。また、データベース(`data/calsync.db`)を失った状態から再構築した場合、それ以前の終了済み予定のブロッカーは管理外の履歴となり `accounts remove` では削除されません(各カレンダーでタイトル(既定「予定あり」)を検索して手動削除)。
