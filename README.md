...
- **できること**: ダッシュボード(デーモン状態・アカウント別の最終同期・doctor 実行・launchd の停止/起動/再起動)、カレンダー(全アカウントの実予定を週/月ビューで 1 つに重ねて表示。ブロッカーは除外・free の予定や digest 専用カレンダーの予定は含む。アカウントごとに色分けし凡例を表示、イベントクリックで既定ブラウザを開く。**launchd 管理下限定**)、設定(`data/calsync.yaml`)のフォーム編集(コメントを保持したまま保存・外部変更を mtime で検出・保存後は再起動を誘導)、アカウント追加ウィザード(前提チェック → OAuth 認可 → カレンダー選択 → YAML 追記 → 再起動誘導)、自動更新(起動時に GitHub Releases の最新版を確認し、ヘッダのバナーから署名済みアップデートをダウンロード → 再起動。ヘッダの「更新確認」リンクから手動確認も可能)
- **できないこと**: アカウントの削除(危険度が高いため非対応。[アカウントの削除](#アカウントの削除)または `calsync-uninstall` スキルを使ってください)、`calsync sync` / `calsync reconcile` の手動実行
