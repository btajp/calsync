...
- **できること**: ダッシュボード(デーモン状態・アカウント別の最終同期・doctor 実行・launchd の停止/起動/再起動・**フルリコンサイルの実行**)、カレンダー(全アカウントの実予定を週/月/スケジュール(リスト)ビューで 1 つに重ねて表示。ブロッカーは除外・free の予定や digest 専用カレンダーの予定は含む。アカウントごとに色分けし凡例を表示、イベントクリックでアプリ内モーダルに詳細(タイトル・日時・アカウント・説明文)を表示し、会議 URL があれば「会議に参加」、「カレンダーで開く」で元カレンダーをブラウザ表示。**launchd 管理下限定**)、設定(`data/calsync.yaml`)のフォーム編集(コメントを保持したまま保存・外部変更を mtime で検出・保存後は再起動を誘導)、アカウント追加ウィザード(前提チェック → OAuth 認可 → カレンダー選択 → YAML 追記 → 再起動誘導。完了画面からフルリコンサイルの即時実行も可能)、自動更新(起動時に GitHub Releases の最新版を確認し、ヘッダのバナーから署名済みアップデートをダウンロード → 再起動。ヘッダの「更新確認」リンクから手動確認も可能)、**メニューバー常駐**(トレイアイコンに次の予定(60分未満は「N分後」、当日は時刻、翌日は「明日HH:MM」、7日以内は「M/D」)を表示。クリックで今日〜+7日のスケジュール一覧をポップオーバー表示し、項目クリックでパネル内に詳細表示(「← 戻る」でリストへ)、「アプリを開く」でメインウィンドウを表示、「終了」でアプリ全体を終了)
