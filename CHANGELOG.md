...
- **デスクトップアプリ: 長時間の予定にマウスが触れると中に重なっている予定が隠れてホバーできなくなる問題を修正**。ホバー全幅展開の対象を「自分の前に誰も重なっていない(最前面の)予定」に限定した。背面側の予定(長時間ブロック等)は展開せず、全文はツールチップとクリックで確認する

## [0.6.0] - 2026-07-29

### Added

- **デスクトップアプリ: 同時開始の予定を 1 つの「リスト箱」に統合**。同じ時刻に始まる複数の予定は列を分割せず 1 つの箱にまとめ、中を「アカウント色チップ+タイトル+時刻範囲」の全幅 1 行リストで表示する(週/日ビュー。月/スケジュールは連結タイトル。2 件で約 2 倍、3 件で約 3 倍の文字数が読める — 30 分×2 件は 2 行目が部分表示になるためホバー展開かバッジで補完)。件数は「N件」バッジ・行間の区切り線・左端のレール(本数=件数、長さ=各予定の長さ)で分かる。行クリックでその予定の詳細を開く
- **デスクトップアプリ: 予定にマウスを乗せると全幅に展開して全文表示**(週ビュー。クリック不要の中間層。離すと元に戻る)
- **デスクトップアプリ: タイトル先頭の【…】接頭辞(【社内】等)を小さなチップに変換**し、識別に効く文字を行の先頭に出す(週ビューの単独予定とリスト箱の行)

### Changed

- 重なり詰め(1 段 5%)の inline 上書きから `!important` を除去(ホバー全幅展開が勝てるようにするため。上書き自体は維持)

## [0.5.6] - 2026-07-29

### Changed

- **デスクトップアプリ: 週ビューの時刻表示を「開始 - 終了」の範囲に戻す**(0.5.5 で開始時刻のみにしていた)。タイトル先頭は維持し、幅が足りないときは末尾(時刻側)から自然にクリップされる — 表示できるスペースがあるときは終了時刻も見えるように

## [0.5.5] - 2026-07-29

### Changed

- **デスクトップアプリ: 被っている予定の左インセットを「1 段 5%」に圧縮**。長時間ブロックと重なる予定が右半分に潰れていたのを、時間差の重なりはほぼ全幅で右へ 5% ずつずらすカスケードに変更(開始時刻が同じ予定同士は従来どおり横並びを維持 — 5% 詰めでは後の予定が前を覆って読めなくなるため)
- **デスクトップアプリ: 週ビューの予定はタイトルを先頭に表示**(Google カレンダーと同じ)。時刻は開始時刻のみをタイトルの後ろに控えめに表示する(グリッド上の位置と枠の下端で時間はわかる。終了時刻はツールチップ・詳細で確認可)。月/スケジュールビューとメニューバーのポップオーバーは従来どおり時刻先頭
- **デスクトップアプリ: 週ビューのスロットを少し高くして(1.5em → 2em)、30 分予定でも 1 行(タイトル)が確実に読めるように**

## [0.5.4] - 2026-07-29

### Fixed

- **デスクトップアプリ: カレンダーの予定タイトルが枠の下へあふれて隣の予定に重なる問題を修正**。枠内に収まらない文字はクリップし、全文はホバーのツールチップ(タイトル+会議 URL)とクリックの詳細で見られる(Google カレンダーと同じ流儀)
- **デスクトップアプリ: 現在時刻ラインのドットと半透明ラインが実線から 2px ずれる問題を修正**(疑似要素の配置基準がボーダー内側のパディングボックスであることを考慮していなかった)

### Changed

- **デスクトップアプリ: 被っている予定の幅を列の右端まで拡大**。長時間の予定と重なる予定が半分以下の幅に潰されていたのを、後の予定が前の予定の右側に重なる Google 風カスケードにして描画幅を確保した

## [0.5.3] - 2026-07-28

### Changed

- **デスクトップアプリ: Zoom の参加ボタンアイコンを公式アプリタイルに変更**(公式ブランドアセットの PNG を同梱。CSP が外部画像を禁止するためリンクではなく同梱)。Meet / Webex は引き続き simple-icons、Teams は頭文字バッジ
...
- **デスクトップアプリ: カレンダーの現在時刻ラインを Google カレンダー風に強化**。今日の列は 2px の実線+左端に丸ドット、週全体には半透明の細いラインを重ねて表示し、1px 線の見えにくさを解消した
- **デスクトップアプリ: 参加ボタンに会議サービスの種別を表示**。Zoom / Google Meet / Microsoft Teams / Webex を会議 URL のホストから判定し、詳細ビューのボタンは「Zoom で参加」のようにサービス名+ブランド近似色のドット、ポップオーバーの行ボタンは色ドット+ツールチップで示す(ロゴ画像は商標のため同梱しない)

## [0.5.0] - 2026-07-28

### Added

- **デスクトップアプリ: 現在時刻の表示を追加**。カレンダータブの週/日ビューに現在時刻の水平線(FullCalendar の nowIndicator)、メニューバーのポップオーバーには今日のグループ内に「現在時刻」の赤い水平線(終日・開始済みの直後 = これから始まる最初の予定の直前)を表示する。ポップオーバーの自動スクロールもこの水平線を基準に合わせる
- **デスクトップアプリ: Zoom の会議はブラウザを経由せず Zoom デスクトップアプリで直接開くように**。「会議に参加」/「参加」ボタンは、Zoom の会議 URL(`/j/<会議番号>` 形式。vanity ホスト・zoomgov 対応)を `zoommtg://` スキームに変換して Zoom アプリを直接起動する(パスコードも引き継ぐ)。Zoom アプリが無い等で開けない場合は従来どおりブラウザで開く。あわせて shell open の許可スコープを `https://` + `zoommtg://` に限定(従来の既定は http/mailto/tel も許可していた)

### Changed

- **デスクトップアプリ: 今日の強調を強化**。カレンダータブの今日の背景色を濃くし、列見出し/日付番号をピル型で強調。「今日」ジャンプボタン(ツールバー左・従来からある機能)も、今日が表示範囲外のときに青色で目立つようにした
- **デスクトップアプリ + appserver: 予定の読み込みを高速化**。(1) 予定収集(`Engine.CollectWindow`)のアカウント間取得を並列化(dedupe の決定的な設定順マージは維持)。(2) `GET /api/events` のキャッシュに stale-while-revalidate を導入 — TTL(60秒)切れでも 30 分以内の内容なら `stale: true` を付けて即座に返し、バックグラウンドで取り直す。フロントは stale 応答の約 4 秒後に 1 回だけ再取得して最新化する(カレンダータブ・ポップオーバー共通)

## [0.4.2] - 2026-07-28

### Added

- **デスクトップアプリ: メニューバーのポップオーバーのリスト各行に「参加」ボタンを追加**。会議 URL(https)がある予定は、詳細ビューを開かずにその場で会議に参加できる(行クリックは従来どおり詳細ビュー)

## [0.4.1] - 2026-07-28

### Changed

- **デスクトップアプリ: 予定詳細の「会議に参加」ボタンをタイトル右側に移動**。長い説明文では最下部のボタン行までスクロールが必要だったため、最頻用アクションを常に見える位置に出した。あわせて詳細ビューは説明文だけを内部スクロールにし、タイトル・日時・下部の「カレンダーで開く/閉じる」が常に見えるようにした
- **デスクトップアプリ: メニューバーのポップオーバーを開いたとき、今日の開催中/次の予定まで自動スクロールするように**。パネルは使い回されるため前回のスクロール位置が残っていた(定期更新ではスクロールせず、閲覧中に位置が飛ばないようにしている)

## [0.4.0] - 2026-07-24

### Added

- **デスクトップアプリ: アプリ内で予定詳細を表示できるように**。カレンダータブのイベントクリック、メニューバーのポップオーバーのスケジュール項目クリックのいずれも、ブラウザで元カレンダーを開く代わりにアプリ内で詳細(タイトル・日時・アカウント・説明文)を表示するようにした。説明文は改行を保持しつつ URL を自動リンク化し(URL 構成文字で終端し、末尾の約物・日本語の地の文は URL に含めない)、https のリンクだけをクリック可能として既定ブラウザで開く(http やパース不能な URL はテキスト表示)。会議 URL がある予定には「会議に参加」ボタンを、従来の「ブラウザで開く」動作は「カレンダーで開く」として退避した。カレンダータブはモーダル(背景クリック・✕・Esc で閉じる)、ポップオーバーはパネル内切り替え(「← 戻る」でリストへ)で、同じ表示部品(`EventDetail`)を共用する。`GET /api/events` の各イベントに `description`(プレーンテキスト)を追加

### Changed

- **デスクトップアプリ: アプリアイコンとメニューバーアイコンを刷新**。アプリアイコンは青グラデーションの squircle にカレンダー+同期矢印のモチーフ、メニューバーはカレンダー型のテンプレートアイコン(ライト/ダーク自動追従)に変更

## [0.3.2] - 2026-07-23

### Fixed

- **デスクトップアプリ: 1 回のクリックでポップオーバーが開いて即閉じることがある問題を修正**。トレイの Click イベントは 1 クリックで buttonState Down/Up の 2 回届くため、Down で開いた直後の Up がトグル判定に入っていた。Up のみ処理し、さらに表示直後 300ms はトグル閉じを受け付けない(マウスチャタリング対策)
- **デスクトップアプリ: ポップオーバーに終了済みの予定を表示しないように**(時刻あり予定は終了時刻を過ぎたら非表示。開催中と終日は表示)

## [0.3.1] - 2026-07-23

### Fixed

- **デスクトップアプリ: トレイ再クリックでポップオーバーを閉じられるように**(トグル動作。macOS の blur → hide → Click の到着順序による「閉じてもすぐ再表示される」挙動を、hide 直後クリックの抑制で解消)
- **デスクトップアプリ: カレンダーの前/次ボタンが豆腐(欠字)になる問題を修正**(FullCalendar のアイコンフォントは data: URL 埋め込みだが、0.3.0 の CSP に `font-src` が無くブロックされていた。`font-src 'self' data:` を追加)
...
- **デスクトップアプリ + appserver: 最終ホールレビュー指摘(Fix 1〜3)に一括対応**: メンテナンス(リコンサイル)実行中にアプリを終了すると、`launchctl bootout` 済みのデーモンが再起動されないまま残ってしまう問題を修正。フロント(`quit-app` リスナー)はメンテナンス実行中の終了を確認ダイアログ(「それでも終了」)で止めるようにし、appserver 側(`Serve` の graceful shutdown 経路)にも Cmd+Q・stdin EOF 等の確認ダイアログを経由しない終了向けの最終防衛として、fresh な 60 秒予算での `launchctl bootstrap` best-effort 実行を追加した(バックグラウンドの `runMaintenanceWindow` 自身が持つ bootstrap 保証は、Go ランタイム終了に間に合わない場合があるため) / データディレクトリ変更後、ポップオーバーが変更前の古い port/token を掴んだまま死んだ接続を握り続ける問題を修正。メインウィンドウの api 確立時に `api-info` イベントを能動送信するように(パネル側は受信のたびにクライアントを差し替える既存実装で完結) / Retina(scaleFactor 2 以上)ディスプレイでポップオーバーの表示位置がずれる問題を修正。物理ピクセルのトレイ矩形を、クリック座標が属するモニタの scaleFactor で論理ピクセルへ変換してから位置計算するように / `POST /api/daemon/{action}` にメンテナンス実行中の 409(`maintenance_in_progress`)ガードを追加(フロントの無効化はクライアント側の見た目のガードにすぎず、サーバ側でも同じ 409 を返すようにした)
