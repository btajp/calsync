...
- **終わった予定のブロッカーが日次リコンサイルで削除されていた問題を修正**。同期ウィンドウ(現在〜未来3ヶ月)のスライド時に過去側へ抜けたブロッカーも「範囲外」として掃除していたのを、未来側の範囲外のみ掃除するように変更。終わった予定の「予定あり」ブロッカーは履歴としてカレンダーに残る(元予定が削除された場合は従来どおり追従削除。ウィンドウ内から過去へ予定が移動された場合の移動元ブロッカー削除も従来どおり)
