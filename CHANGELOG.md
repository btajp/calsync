...
- **デスクトップアプリ: 同時開始の予定を 1 つの「リスト箱」に統合**。同じ時刻に始まる複数の予定は列を分割せず 1 つの箱にまとめ、中を「アカウント色チップ+タイトル+時刻範囲」の全幅 1 行リストで表示する(2 件で約 2 倍、3 件で約 3 倍の文字数が読める)。件数は「N件」バッジ・行間の区切り線・左端のレール(本数=件数、長さ=各予定の長さ)で分かる。行クリックでその予定の詳細を開く
