...
- **終わった予定のブロッカーが日次リコンサイルで削除されていた問題を修正**。同期ウィンドウ(現在〜未来3ヶ月)のスライド時に過去側へ抜けたブロッカーも「範囲外」として掃除していたのを、未来側の範囲外のみ掃除するように変更。終わった予定の「予定あり」ブロッカーは履歴としてカレンダーに残る(元予定が削除された場合は削除通知が届く経路では従来どおり追従削除。ウィンドウ内から過去へ予定が移動された場合の移動元ブロッカー削除も従来どおり)
- 上記に伴う保護: 過去向けに新規ブロッカーを作らないガードを suppressed 昇格・pending 解決にも追加。Microsoft のブロッカー列挙(ListBlockers)に時刻下限(`end/dateTime ge ウィンドウ開始`)を追加し、過去ブロッカーの蓄積で日次リコンサイルの API コストが際限なく増えないようにした(組合せ $filter が拒否された場合は従来の列挙へ自動フォールバック)。既知の限界: DB を失って再構築した場合、過去分は「管理外の履歴」となり削除追従・`accounts remove` の対象外(手動削除で対応)

## [0.6.2] - 2026-07-31

### Fixed

- **デスクトップアプリ: マウスオーバーが分かりにくい問題を修正**。すべての予定にホバー時の軽い明度変化を追加(0.6.1 で背面の予定から展開を外した結果、ホバーのフィードバックが無くなっていた)
